...
#   channelRelease = "nixos-19.09pre182429.98e3b90b6c8";
...
in
{
  ...
  services.xserver = {
    ...
    # xkbModel = "pc105";
    xkbVariant = "dvorak";
    # xkbDir = "${pkgs.new_xkeyboardconfig}/etc/X11/xkb"; # xserver-wrapper from xkeyboard both with/without patch
    # displayManager.xserverArgs = [setting on xbkDir fixed it];
    desktopManager = {
      mate.enable = true;
      #default = "mate";
    };
  ...
  nixpkgs.overlays = [
    (self: super: {
      ...
      libxklavier = super.libxklavier.overrideAttrs (old: {
        ...
        ]; # from xkeyboard with patch (zmhss5)  
      });
      #ckbcomp = super.ckbcomp.overrideAttrs (old: {	
      #  patchPhase = ''
      #    substituteInPlace Keyboard/ckbcomp --replace "/usr/share/X11/xkb" "${self.new_xkeyboardconfig}/share/X11/xkb"
      #    substituteInPlace Keyboard/ckbcomp --replace "rules = 'xorg'" "rules = 'base'"
      #  '';
      #}); # from xkeyboard with patch (zmhss5)

      #new_libxkbcommon = super.libxkbcommon.overrideAttrs (old: {
      #  mesonFlags = old.mesonFlags ++ [
      #    "-Dxkb-config-root=${self.new_xkeyboardconfig}/share/X11/xkb"
      #  ];  
      #}); # from xkeyboard with patch (zmhss5) - cause of rebuilding everything

      #new_gtk3 = super.gtk3.override {
      #  libxkbcommon = self.new_libxkbcommon;
      #}; not working

      #plymouth = super.plymouth.override {
      #  gtk3 = self.new_gtk3;
      #}; not working
